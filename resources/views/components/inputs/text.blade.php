@if (isset($bag))
    <input id="{{ $bag . '-' . $name }}" type="{{ isset($type) ? $type : 'text' }}" class="c-text-input form-control{{ $errors->$bag->has($name) ? ' is-invalid' : '' }}" name="{{ $name }}" {!! isset($placeholder) ? "placeholder='$placeholder'" : '' !!} value="{{ isset($value) ? $value : '' }}">
